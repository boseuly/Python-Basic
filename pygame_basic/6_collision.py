...
# 적 enemy 캐릭터
enemy = pygame.image.load("/Users/elena/PythonWorkspace/pygame_basic/enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해옴
enemy_width = enemy_size[0] # 캐릭터의 가로 크기
enemy_height = enemy_size[1] # 캐릭터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)# 화면 가로의 절반 크기에 해당하는 곳에 위치 (가로)
enemy_y_pos = (screen_heigth / 2) - (enemy_height / 2) # 세로 화면의 절반 크기에 해당하는 곳에 위치 (세로)
...
while running: 
    ...
    # 10fps : 1초 동안에 10번 동작
    # 20fps : 1초 동안에 20번 동작 
    # 프레임이 달라진다고 해서 게임 이동 속도가 달라지면 안 됨 => 조정해줄 필요 있음
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # 사각형 기준으로 충돌이 존재하는지 확인 가능
        print("충돌했어요")
        running = False
